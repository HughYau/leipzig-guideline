...
function renderToc(sections) {
  tocEl.innerHTML = [
    `<a class="level-1" href="#cover">首页</a>`,
    ...sections.map((section) => {
      const cls = section.level === 1 ? "level-1" : "level-2";
      return `<a class="${cls}" href="#${section.id}" data-section="${section.id}">${escapeHtml(section.title)}</a>`;
    }),
  ].join("");
}
...
function renderBlock(block) {
  switch (block.type) {
    case "paragraph":
      return `<p>${inlineLinks(block.text)}</p>`;
    case "list":
      return `<ul>${block.items.map((item) => `<li>${inlineLinks(item)}</li>`).join("")}</ul>`;
    case "table":
...
function renderCardBlock(className, block) {
  return `
    <aside class="content-card ${className}">
      ...
      <ul>${block.items.map((item) => `<li>${inlineLinks(item)}</li>`).join("")}</ul>
    </aside>
  `;
}
...
function inlineLinks(text) {
  ...
  return escaped.replace(/(https?:\/\/[^\s<]+)/g, (url) => {
    ...
    return `<a href="${cleanUrl}" target="_blank" rel="noreferrer">${cleanUrl}</a>${escapeHtml(suffix)}`;
  });
}
